...
{
    [RoPrimitive("Circle2D", typeof(CircleData))]
    public class Circle2DPrimitive : IPrimitiveHandler
    {
        ...
        public static void RenderCircle(RagnarokPrimitive primitive, MeshBuilder mb)
        {
            ...
            for (var i = 0f; i < span; i += data.ArcAngle)
            {
                ...
                var inner1 = new Vector3(c1 * innerSize, s1 * innerSize, 0f);
                var inner2 = new Vector3(c2 * innerSize, s2 * innerSize, 0f);

                var point1 = new Vector3(c1 * data.Radius, s1 * data.Radius, 0f);
                var point2 = new Vector3(c2 * data.Radius, s2 * data.Radius, 0f);
