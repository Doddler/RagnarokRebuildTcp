...
namespace Assets.Scripts.SkillHandlers.Handlers.Knight
{
    ...
    public class BrandishSpearHandler : SkillHandlerBase
    {
        ...
        {
            HoldAttackMotionForCast(src, castTime + 1, 1f);
        ...
        public override void ExecuteSkillTargeted(ServerControllable src, ref AttackResultData attack)
        {
            ...
            var id = CameraFollower.Instance.EffectIdLookup["BrandishSelf"];
            CameraFollower.Instance.AttachEffectToEntity(id, src.gameObject, src.Id);

            if (attack.Target != null)
            {
                id = CameraFollower.Instance.EffectIdLookup["BrandishTarget"];
                CameraFollower.Instance.AttachEffectToEntity(id, attack.Target.gameObject, src.Id);
            }
        }
    }
}