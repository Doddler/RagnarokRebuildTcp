...
{
    [SkillHandler(CharacterSkill.MagnusExorcismus)]
    public class MagnusExorcismusHandler : SkillHandlerBase
    {
        public override void StartSkillCasting(ServerControllable src, Vector2Int target, int lvl, float castTime)
        {
            ...
            if(target != Vector2Int.zero)
                CastTargetCircle.Create(src.IsAlly, targetCell, 4, castTime);
        }
        ...
        public override void ExecuteSkillGroundTargeted(ServerControllable src, ref AttackResultData attack)
        {
            ...
            CameraFollower.Instance.CreateEffectAtLocation("MagnusExorcismus", attack.TargetAoE.ToWorldPosition(), new Vector3(1.5f, 1.5f, 1.5f), 0);
